...
class preTradeClient:
{
public:
	static preTradeClient& getInst();
//	static bool connect(std::string &host, int port, std::string &usr, std::string &passwd);
	static bool sendRequest(std::string &msg);
	static bool init(std::string host, int port, std::string usr, std::string passwd);

private:
	preTradeClient();
	preTradeClient(preTradeClient const&);// Don't Implement
	void operator=(preTradeClient const&);// Don't implement

public:
	preTradeClient(preTradeClient const&) = delete;
	void operator=(preTradeClient const&) = delete;
}
