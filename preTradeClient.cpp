...
#include <chrono>
#include <amqpcpp.h>
#include "pretTradeClient.h"

preTradeClient& pretTradeClient::getInst();
{
	static preTradeClient inst;
	return instance;
}

bool pretTradeClient::init(std::string host, int port, std::string usr, std::string passwd)
{
	std::stringstream ss;
	...
	AMQP::TcpConnection connection(handler,
			AMQP::Address(host, port, AMQP::Login(usr, passwd),
					"/"));
	...
	channel.consume("", AMQP::noack).onReceived(
...
//	msgHandler handler;
//	AMQP::TcpConnection connection(handler,
//			AMQP::Address("localhost", 5672, AMQP::Login("guest", "guest"),
//					"/"));
//	AMQP::TcpChannel channel(&connection);
//	channel.onError([&handler](const char *message) {
//		std::cout << "Channel error: " << message << std::endl;
